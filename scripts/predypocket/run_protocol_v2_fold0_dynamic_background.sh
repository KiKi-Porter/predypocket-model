...
GPU_ID="${GPU_ID:-5}"
RUN_DIR="$REPO_ROOT/outputs/predypocket_protocol_v2/training/fold0/dynamic"
...
if [[ -f "$RUN_DIR/training.pid" ]]; then
  ...
  if [[ -n "$existing_pid" ]] && kill -0 "$existing_pid" 2>/dev/null; then
    echo "Protocol-v2 dynamic fold0 is already running as PID $existing_pid" >&2
    exit 3
  fi
fi
if [[ -e "$RUN_DIR/history.json" || -e "$RUN_DIR/best_checkpoint.index" ]]; then
  echo "Refusing to overwrite existing protocol-v2 dynamic fold0 results" >&2
  exit 2
fi
...
echo "Started protocol-v2 fold0 dynamic PID $! on physical GPU $GPU_ID"
